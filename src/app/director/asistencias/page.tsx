...
import { listAsistencias } from '@/lib/actions/admin'

export default function AsistenciasPage() {
  const [asistencias, setAsistencias] = useState<any[]>([])
  const [fecha, setFecha] = useState('')

  const loadAsistencias = async (f?: string) => {
    const data = await listAsistencias(f || undefined)
    setAsistencias(data)
  }
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      loadAsistencias(fecha || undefined)
    }, 300)
                ...
                <tbody>
                  {asistencias.map((a) => (
                    <tr key={a.id} className="border-b border-[var(--border)] last:border-0">
                      ...
                      <td className={`py-3 font-medium ${estadoColor(a.estado)}`}>
